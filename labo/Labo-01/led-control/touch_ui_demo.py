...
MODE_LABELS = [
    "Mode 0: Chenillard",
    "Mode 1: Bargraphe POT",
    "Mode 2: Paires alternees",
    "Mode 3: Toggle global",
]
...
class DemoUI:
    ...
    def _send_command(self, cmd: str):
        ...
        try:
            ...
            self.status_message = f"Commande envoyee: {cmd}"
        except Exception as exc:
    ...
    def _build_buttons(self, h, w):
        ...
        config = [
            ("MODE -", left_x, start_y, 1),
            ("MODE +", right_x, start_y, 1),
            ("PAUSE/RESUME", left_x, start_y + 4, 3),
    ...
    def _draw(self):
        ...
        title = " Demo ESP32: ecran tactile + LEDs + boutons + pot "
        self.stdscr.attron(curses.A_BOLD)
        self.stdscr.addstr(0, max(0, (w - len(title)) // 2), title[: w - 1])
        self.stdscr.attroff(curses.A_BOLD)

        mode_text = f"Mode courant: {self.selected_mode} - {MODE_LABELS[self.selected_mode]}"
        run_text = "Animation: ACTIVE" if self.is_running else "Animation: PAUSE"
        self.stdscr.addstr(1, 1, mode_text[: w - 2], curses.A_BOLD)
        self.stdscr.addstr(2, 1, run_text[: w - 2], curses.A_BOLD)

        feedback_title = "--- Feedback ESP32 ---"
        self.stdscr.attron(curses.A_UNDERLINE)
        self.stdscr.addstr(max(3, h // 2 - 10), 1, feedback_title[: w - 2])
        self.stdscr.attroff(curses.A_UNDERLINE)

        start_row = max(4, h // 2 - 9)
        ...
        self.stdscr.addstr(h - 1, 1, "Touches: q quitter | m mode+ | p pause/resume | s status")
        self.stdscr.refresh()
    ...
    def _handle_action(self, label):
        if label == "MODE -":
            self._set_mode((self.selected_mode - 1) % 4)
        elif label == "MODE +":
            self._set_mode((self.selected_mode + 1) % 4)
        elif label == "PAUSE/RESUME":
            self._send_command("TOGGLE_RUN")
    ...
    def run(self):
        ...
        while self.running:
            ...
            if ch == ord("q"):
                ...
                self._handle_action("MODE +")
            elif ch == ord("p"):
                self._handle_action("PAUSE/RESUME")
            elif ch == ord("s"):
...
def main(stdscr):
    ...
    touch_reader.start()
    ui = DemoUI(stdscr, touch_reader, touch_queue)
    ui.run()
